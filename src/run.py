...
def sample_questions(con, n: int):
    rows = con.execute(
        "SELECT question_id, prompt FROM questions ORDER BY created_at DESC LIMIT ?",
        (n,)
    ).fetchall()
    return [(r["question_id"], r["prompt"]) for r in rows]

def run_benchmark(client, con, model: str, base_url: str, temperature: float, n: int, alpha: float = 0.2):
    # create run record
    run_id = new_id()
    con.execute(
        "INSERT INTO runs(run_id, run_at, base_url, model, temperature, n_questions) VALUES(?,?,?,?,?,?)",
        (run_id, now_iso(), base_url, model, float(temperature), int(n))
    )
    con.commit()

    qs = sample_questions(con, n)
    scores = []
    ...
    for qid, q in qs:
        ...
        j = judge_answer(client, model=model, question=q, answer=answer)
        score = float(j["score"])
